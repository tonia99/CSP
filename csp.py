...
class csp:
    ...
    def constraints(self,A, a,assignment):
        ...
        for item in l:
          if ([k for k, v in self.confederation.items() if str(item) in v][0] == ("Group6")):
              count=count+1

        for item in l:

          if ([k for k, v in self.confederation.items() if str(item) in v][0] == "Group6"):
             if ([k for k, v in self.confederation.items() if str(A) in v] == [k for k, v in self.confederation.items() if str(item) in v]  and count>=2 ) :
                return True
          elif  ([k for k, v in self.confederation.items() if str(item) in v][0] != "Group6"):
             if ([k for k, v in self.confederation.items() if str(A) in v] == [k for k, v in self.confederation.items() if str(item) in v]):
    ...
    def recursive_backtracking(self,assignment, csp):
        ...
        var1 = self.lcv(var, assignment, csp)
    ...
    def numconflicts(self, A, a, assignment):
        ...
        for item in l:
            ...
            if ([k for k, v in self.confederation.items() if str(item) in v][0] == "Group6"):
                if ([k for k, v in self.confederation.items() if str(A) in v] == [k for k, v in
                                                                                      self.confederation.items() if
...
def get_details():
    ...
    input_given = list(map(lambda s: s.strip(), input_given))
